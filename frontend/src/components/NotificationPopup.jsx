...
export default function NotificationPopup({ open, onClose, notification }) {
    ...
    const { sender, data, created_at, description, title, tech_info, tech_id } = notification;

    // Specs odesílatele
    const profile_job = sender?.specs?.length > 0
    ...
    const popupContent = (
        <>
            <div className="popover-backdrop" onClick={onClose}>
                <div className="notification-popup" role="dialog" onClick={(e) => e.stopPropagation()}>
                    {tech_info?.image ? (
                        <div className="left-right-container">
                            <div className="left">
                                <p className="body_smallest strong date">{timestamp}</p>
                                <h2>{renderTitle()}</h2>

                                <div className="profile smaller_opacity" onClick={() => handleNavigate(buildRoute(ROUTES.USER_DETAIL, { id: sender?.id }))}>
                                    <img
                                        className="profile_img"
                                        src={sender?.profile_image_url || ASSETS.default_avatar}
                                        alt=""
                                    />
                                ...
                                <p className="body_base message-text">{description}</p>
                            </div>
                            <div className="right">
                                <img
                                    ...
                                    src={tech_info.image}
                                    alt={tech_info.title}
                                    onClick={() => handleNavigate(`/tech/item/${tech_info.id}`)}
                                />
                            </div>
                        </ div>
                    ) : (
