import { faCircleQuestion, faGears, faPeopleGroup, faStar } from '@fortawesome/free-solid-svg-icons';
...
const AdminReportTable = ({ reports, onResolve }) => {
  ...
  const getTargetIcon = (type) => {
    ...
    switch (model) {
      case 'User': return <FontAwesomeIcon icon={faPeopleGroup} />;
      case 'Item': return <FontAwesomeIcon icon={faGears} />;
      case 'ReviewUser':
      case 'ReviewItem': return <FontAwesomeIcon icon={faStar} />;
  ...
  return (
    <div className="report-table-wrapper">
      <table className="admin-custom-table">
        ...
        <tbody>
          {reports?.map((report) => (
            <tr key={report.id} className={getReportUrgency(report.created_at)}>
              <td className="col-date">
                <div className="date-wrapper">
                  <span className="date-main">{formatDateTime(report.created_at)}</span>
                  <span className="date-relative">
                    před {Math.round((new Date() - new Date(report.created_at)) / (1000 * 60 * 60))} h
                  </span>
                </div>
              </td>
              <td className="col-type">
                <div className="type-badge">
                  <i className={`fa-solid ${getTargetIcon(report.target_type)}`}></i>
                  {getTargetIcon(report.target_type)}
                  <span>
                    {(() => {
                      const model = report.target_type.split('\\').pop();
                      return typeTranslations[model] || model; // Pokud překlad neexistuje, vrátí původní název
                    })()}
                  </span>
                </div>
              </td>
                ...
                <a href={getTargetLink(report)} target="_blank" rel="noreferrer" className="target-link">
                  <div className="target-info">
                    ...
                    {(() => {
                      ...
                      const strikes = target.strikes_count ?? target.user?.strikes_count ?? target.reviewer?.strikes_count;

                      return strikes > 0 ? (
                        <span className="strike-warning">
                          <i className="fa-solid fa-triangle-exclamation"></i> {strikes}x Strike
                        </span>
                      ) : null;
                    })()}
                    ...
                    <small>
                      {
                        ...
                        report.target?.reviewer?.email || // Pokud je cílem Recenze (ReviewItem/ReviewUser)
              ...
              <td>
                <span className={`badge-category ${report.report_category?.toLowerCase().replace(/\s+/g, '-')}`}>
                  {report.report_category}
                </span>
              </td>
              <td className="col-reason">
                <div className="reason-content">{report.reason || 'Bez popisu'}</div>
              </td>
              <td className="col-reporter">
                <a href={`/user/${report.reporter?.id}/profile`} target="_blank" rel="noreferrer" className="reporter-link">
              ...
              <td className="col-actions">
                <div className="actions-stack">
                  <button className="btn-resolve" onClick={() => onResolve(report, 'resolve')}>Řešit</button>
                  <button className="btn-dismiss" onClick={() => onResolve(report, 'dismiss')}>Ignorovat</button>
                </div>
              </td>
            </tr>
          ))}
