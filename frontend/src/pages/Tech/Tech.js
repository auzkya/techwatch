...
import { faCircleMinus, faCirclePlus, faLightbulb, faMagnifyingGlass, faMasksTheater, faStar as faStarFull, faVideo, faWrench } from '@fortawesome/free-solid-svg-icons';
...
const Tech = () => {
    ...
    const [selectedPurpose, setSelectedPurpose] = useState({ value: "all", label: "Vše" });
    const [selectedSorting, setSelectedSorting] = useState({ value: "created_at", label: "Data přidání" });
    ...
    }, [subcategory, selectedLocation.value, searchTerm, minRating, filters, selectedPurpose.value, selectedSorting.value]);
    ...
    const getFirstImage = (imagesData) => {
        ...
        if (Array.isArray(imagesData)) return imagesData[0] || ASSETS.default_item;
        return imagesData;
    };
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (isOpen && !selectRef1.current?.contains(event.target)) setIsOpen(false);
            if (isOpen2 && !selectRef2.current?.contains(event.target)) setIsOpen2(false);
            if (isOpen3 && !selectRef3.current?.contains(event.target)) setIsOpen3(false);
        };
        document.addEventListener("mousedown", handleClickOutside);
    ...
    /*const memoizedItems = useMemo(() => {
        return items.map((item, index) => (
            <Item
                ...
                onClick={handleTechClick} // Stabilní reference
            />
        ));
    }, [items, lastItemRef, handleTechClick]);*/


    const allowedSubcategories = ["light", "sound", "video", "rigging_stage", "scenography"];
        ...
        <>
            ...
            <div className="search_bar-container">
                <div className="search_bar-box">
                    ...
                    <input className="search_bar" placeholder="Vyhledávejte techniku" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
            </div>
            ...
            {!subcategory && (
                ...
                <div className="workers-nav">
                    ...
                    <ButtonSubcategory
                        ...
                        text="Světla"
                        onClick={() => navigate(buildRoute(ROUTES.TECH_CATEGORY, { subcategory: "light" }))}
                    />
                    <ButtonSubcategory
                        key={"sound"}
                        ...
                        text="Zvuk"
                        onClick={() => navigate(buildRoute(ROUTES.TECH_CATEGORY, { subcategory: "sound" }))}
                    />
                    <ButtonSubcategory
                        key={"video"}
                        icon={<FontAwesomeIcon icon={faVideo} />}
                        text="Video"
                        onClick={() => navigate(buildRoute(ROUTES.TECH_CATEGORY, { subcategory: "video" }))}
                    />
                    <ButtonSubcategory
                        key={"rigging_stage"}
                        icon={<FontAwesomeIcon icon={faWrench} />}
                        text="Rigging & stage"
                        onClick={() => navigate(buildRoute(ROUTES.TECH_CATEGORY, { subcategory: "rigging_stage" }))}
                    />
                    <ButtonSubcategory
                        key={"scenography"}
                        icon={<FontAwesomeIcon icon={faMasksTheater} />}
                        text="Scénografie"
                        onClick={() => navigate(buildRoute(ROUTES.TECH_CATEGORY, { subcategory: "scenography" }))}
                    />
                </div>
            )}
            ...
            <div className="list-container">
                <div className="all-filters">
                    <div className="filters">
                        ...
                        {/* MNOŽSTVÍ */}
                        <label className="body_base label-move">Min. množství</label>
                            ...
                            <div className="stepper-input-wrapper">
                                ...
                                <div className="stepper-input-wrapper-btn-container">
                                    <FontAwesomeIcon icon={faCircleMinus}
                                        ...
                                        onClick={() => setFilters(prev => ({ ...prev, quantity: Math.max(1, prev.quantity - 1) }))}
                                        disabled={filters.quantity <= 1} />
                                    <FontAwesomeIcon icon={faCirclePlus}
                                        className="stepper-btn"
                                        onClick={() => setFilters(prev => ({ ...prev, quantity: prev.quantity + 1 }))}
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        {/* HODNOCENÍ ZAŘÍZENÍ */}
                        <label htmlFor="rating" className="body_base label-move">Hodnocení zařízení</label>
                        <div id="rating" className="checkbox-container checkbox-container-filter">
                            <input type="checkbox" id="visible" className="custom-checkbox" checked={minRating} onChange={() => setMinRating(!minRating)} />
